...
class Cache:
    ...
    def get_issuetypes_from_system_cache(self) -> list[dict[str, Any]] | None:
        if self.client._no_read_cache:
            raise LookupError('Attempted to access cache when _no_read_cache is set')
        issue_types = self.get_from_system_cache("issuetypes.json")
        if not issue_types: return
        assert isinstance(issue_types, list), f'{issue_types} should be of type list. Got: {type(issue_types)}'
        return issue_types

    def get_from_issue_type_fields_cache(self, filename: str) -> dict[str, Any] | None:
    ...
    def get_createmeta_from_issue_type_fields_cache(self, issuetype_name: str) -> dict[str, Any] | None:
        filename = f"createmeta_{issuetype_name.lower()}.json"
        return self.get_from_issue_type_fields_cache(filename)

    def get_editmeta_from_issue_type_fields_cache(self, issuetype_name: str) -> dict[str, Any] | None:
    ...
    def write_issuetypes_to_system_cache(self, issuetypes):
        self.write_to_system_cache("issuetypes.json", json.dumps(issuetypes))

    def write_to_issue_type_fields(self, filename: str, issuetype_fields: list[dict[str, Any]]) -> None:
    ...
    def write_createmeta(self, issuetype_name: str, issuetype_fields: list[dict[str, Any]]) -> None:
        self.write_to_issue_type_fields(f"createmeta_{issuetype_name.lower()}.json", issuetype_fields)

    def write_editemeta(self, issuetype_name: str, issuetype_fields: list[dict[str, Any]]) -> None:
        self.write_to_issue_type_fields(f"editmeta_{issuetype_name.lower()}.json", issuetype_fields)
