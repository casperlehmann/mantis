...
class TestCache:
    ...
    def test_cache_get_caches_jira_issue(self):
        ...
        decoded = self.jira.cache._get(self.jira.cache.issues, "TASK-1.json")
        assert decoded
    ...
    def test_cache_get_issue_returns_none_when_no_read_cache_is_set(self):
        ...
        with open(self.jira.cache.root / "issues/TASK-1.json", "w") as f:
            json.dump(self.issue_payload, f)
        something = self.jira.cache.get_issue("TASK-1")
    ...
    def test_cache_remove_does_removals(self):
        ...
        with open(self.jira.cache.root / "issues/TASK-1.json", "w") as f:
            json.dump(self.issue_payload, f)
        something_1 = self.jira.cache.get_issue("TASK-1")
        assert something_1 is not None

        # remove with cache.remove
        assert self.jira.cache.remove("issues/TASK-1.json")
        assert not self.jira.cache.remove("issues/TASK-1.json")
        nothing_1 = self.jira.cache.get_issue("TASK-1")
    ...
    def test_cache_remove_issue_does_removals(self):
        ...
        with open(self.jira.cache.root / "issues/TASK-1.json", "w") as f:
            json.dump(self.issue_payload, f)
        something_2 = self.jira.cache.get_issue("TASK-1")
        assert something_2 is not None

        # remove with cache.remove_issue
        self.jira.cache.remove_issue("TASK-1")
        nothing_2 = self.jira.cache.get_issue("TASK-1")
        assert nothing_2 is None

    @pytest.mark.parametrize("identifier", [("createmeta")])
    def test_cache_iter_dir_yields_files(self, identifier: str):
        assert len(list(self.jira.cache.iter_dir(identifier))) == 0
        # cache something
        with open(self.jira.cache.system / f"{identifier}/some_file.json", "w") as f:
            f.write("{}")
        assert len(list(self.jira.cache.iter_dir(identifier))) == 1

    def test_cache_get_issuetypes_from_system_cache(self, requests_mock):
        requests_mock.get(f'{self.jira.api_url}/issue/createmeta/TEST/issuetypes', json=get_issuetypes_response)

        with open(self.jira.cache.system / "issuetypes.json", "w") as f:
            json.dump(CacheData().issuetypes, f)
        retrieved = self.jira.cache.get_issuetypes_from_system_cache()
        assert retrieved
